...
import { Alert, Backdrop, Button, Card, CardMedia, CircularProgress, Divider, Grid, Snackbar, Typography } from '@mui/material';
...
function DestinationsPage() {
    ...
    const getAllRestaurantsInformations = (City) => {
        ...
        setLoading(true);
        axios.get(`/GetAllDestinationsInformations/${City}`, dataConfig)
            .then(response => {
                ...
                <Grid container spacing={5} sx={{ m: 5 }}>
                    {Array.isArray(restaurantGeneralData) && restaurantGeneralData.map((restaurant, index) => (
                        <Grid
                            ...
                            component={Link}
                            to={`/DestinationDetail/${restaurant.location_id}`}
                        >
                            <Grid item xs={12} sm={12} md={1} />
                            <Grid item xs={12} sm={12} md={4}>
